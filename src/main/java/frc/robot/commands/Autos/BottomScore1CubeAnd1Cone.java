...
public class BottomScore1CubeAnd1Cone extends SequentialCommandGroup {
  ...
  public BottomScore1CubeAnd1Cone(Swerve s_Swerve, IntakeSubsystem s_Intake, ArmSubsystem s_Arm, WristSubsystem s_Wrist) {
    ...
    new PathPlannerCommand(s_Swerve, 4, "Back up after cone bottom", true).alongWith(new InstantCommand(() -> s_Arm.shoulderReversed *= -1)).alongWith(new ScoreLow(s_Wrist, s_Arm)).withTimeout(1),
    new PathPlannerCommand(s_Swerve, 2, "Grab cube 1 Bottom").alongWith(new IntakeCube(s_Intake)).withTimeout(3),
    new PathPlannerCommand(s_Swerve, 4, "Go back Bottom 1").alongWith (new ShoulderToHome(s_Arm)).withTimeout(2),
    new PathPlannerCommand(s_Swerve, 2, "Go back Bottom 2").alongWith(new ScoreHigh(s_Arm, s_Wrist)).withTimeout(1.5).alongWith(new InstantCommand(() -> s_Arm.shoulderReversed *= -1)),
    (new OutakeCube(s_Intake)).withTimeout(.5),
