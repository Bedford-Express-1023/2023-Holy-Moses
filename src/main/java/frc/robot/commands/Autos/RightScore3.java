...
public class RightScore3 extends SequentialCommandGroup {
  ...
  public RightScore3(Swerve s_Swerve, IntakeSubsystem s_Intake, ArmSubsystem s_Arm, WristSubsystem s_Wrist) {
    ...
    new PathPlannerCommand(s_Swerve, 4, "Go back top 1").alongWith (new ShoulderToHome(s_Arm)).withTimeout(2),
    new PathPlannerCommand(s_Swerve, 2, "Go back top 2").alongWith(new ScoreHigh(s_Arm, s_Wrist)).withTimeout(1.5).alongWith(new InstantCommand(() -> s_Arm.shoulderReversed *= -1)),
    (new OutakeCube(s_Intake)).withTimeout(.5),
