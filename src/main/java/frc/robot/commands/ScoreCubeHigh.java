...
public class ScoreCubeHigh extends CommandBase {
  ...
  public ScoreCubeHigh(WristSubsystem wrist, ArmSubsystem arm, Trigger button) {
    this.wrist = wrist;
    this.arm = arm;
    this.button = button;
  ...
  public ScoreCubeHigh(WristSubsystem wrist, ArmSubsystem arm) {
    this.wrist = wrist;
    this.arm = arm;
    addRequirements(wrist, arm);
  }
  ...
  public void execute() {
    ...
    if (arm.InPosition()) {
      wrist.wristPosition((arm.shoulderReversed * 52 - arm.shoulderCANCoder.getAbsolutePosition()));
    }
  }
