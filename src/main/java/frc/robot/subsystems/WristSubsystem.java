...
public class WristSubsystem extends SubsystemBase {
  ...
  public void wristPosition() {
    double setpoint = -wristPID.calculate(wristCancoder.getAbsolutePosition(), 
          wristPosition + Math.signum(wristPosition) * wristPositionOverride);
    SmartDashboard.putNumber("WristPID setpoint", setpoint);
    if (wristCancoder.getAbsolutePosition() > 100 && setpoint > 0){
          wristMotor.set(0);
    }
    else if (wristCancoder.getAbsolutePosition() < -90 && setpoint < 0){
          wristMotor.set(0);
    }
